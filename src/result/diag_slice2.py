...
환경 변경 전후 상태(용량 91·T 25, 용량 182·T 30, 충원 확률 0.25·몫 격자 튜닝)를 같은 코드로 다시 돌려 반복별 원자료를 남긴다.
...
STATES = {
  "s2a_cap91_T25": (replace(Cfg(), T=25, p_sup_new=0.5, cover=0.65, b_buy=39.55078125, b_sup=24.70703125, sh_buy=0.3, sh_sup=0.3),
                    "슬라이스 2 종료 시점: 용량 91, T 25, p_sup_new 0.5, 옛 기준 보정 (39.55, 24.71)", OLD, "규칙 기반 (0.3, 0.3)"),
  "s2b_cap182_T30": (replace(Cfg(), T=30, p_sup_new=0.5, b_buy=31.73828125, b_sup=35.25390625, sh_buy=0.3, sh_sup=0.3),
                     "환경 변경 (B)(D) 후: 용량 182, T 30, p_sup_new 0.5, 옛 기준 보정 (31.74, 35.25)", OLD, "규칙 기반 (0.3, 0.3)"),
  "s2c_psup025_grid": (Cfg(), f"p_sup_new 0.25, 용량 273, T 30, 기준 잉여율 보정, 몫 격자 튜닝 결과 {BEST[6:]}", GRID, BEST),
}
