...
STATES = {
  "s2a_cap91_T25": (replace(Cfg(), reps=10, T=25, p_sup_new=0.5, occ=1.0, b_buy=39.55078125, b_sup=24.70703125, sh_buy=0.3, sh_sup=0.3, **OLD_RESP),
                    "슬라이스 2 종료 시점: 용량 91, T 25, p_sup_new 0.5, 옛 기준 보정 (39.55, 24.71)", OLD, ("규칙 기반 (0.3, 0.3)",)),
  "s2b_cap182_T30": (replace(Cfg(), reps=10, T=30, p_sup_new=0.5, occ=0.5, b_buy=31.73828125, b_sup=35.25390625, sh_buy=0.3, sh_sup=0.3, **OLD_RESP),
                     "환경 변경 (B)(D) 후: 용량 182, T 30, p_sup_new 0.5, 옛 기준 보정 (31.74, 35.25)", OLD, ("규칙 기반 (0.3, 0.3)",)),
  "s2c_psup025_grid": (replace(Cfg(), reps=10, occ=1 / 3, b_buy=22.55859375, b_sup=53.41796875, sh_buy=0.0, sh_sup=0.1, **OLD_RESP),
                       "p_sup_new 0.25, 용량 273, T 30, 기준 잉여율 보정, 몫 격자 튜닝 결과 (0.0, 0.1)", GRID, ("규칙 기반 (0.0, 0.1)",)),
  "s2d_twopoint_grid": (replace(Cfg(), reps=10), f"두 점 반응 (0 → 0.5, 기준 잉여율 → 0.85), 실측 점유율 0.602로 용량 151, T 30, 몫 격자 튜닝 결과 {BEST[6:]}", GRID, (BEST,)),
  "s2e_reps30": (Cfg(), "s2d와 같은 시장·정책, 평가 반복 10 → 30 (검정력 확보). (0.1, 0.2)·(0.0, 0.2)는 s2d 평가 seed 결과를 보고 고른 후보",
